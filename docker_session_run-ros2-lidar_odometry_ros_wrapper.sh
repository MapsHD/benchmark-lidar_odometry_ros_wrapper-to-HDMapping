...
IMAGE_NAME='lidar_odometry_ros_wrapper_humble'
...
echo "=== Lidar_odometry_ros_wrapper ROS2 rosbag pipeline ==="
...
ros2 launch lidar_odometry_ros lidar_odometry.launch.py config_file:=/ros2_ws/src/lidar_odometry_ros_wrapper/lidar_odometry/config/mid360.yaml use_sim_time:=true pointcloud_topic:='"$TOPIC"'
'\'' C-m
...
docker run -it --rm \
  ...
  /bin/bash -c "
    source /opt/ros/humble/setup.bash
    source /ros2_ws/install/setup.bash
    ros2 run lidar-odometry-ros-to-hdmapping listener \
      \"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME\" \
      \"$BAG_OUTPUT_CONTAINER/$HDMAPPING_OUT_NAME-lidar-odometry-ros\"
  "
